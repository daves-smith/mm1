...
class vibecarousel extends WP_Widget {
  ...
  function widget( $args, $instance ) {
    ...
    if(isset($ids) && strlen($ids) > 1){
                    $args = array('post_type'=>$type,'orderby'=>$order,'order'=>$sort,'post__in'=>$ids);
                  }else{
                    $args = array('post_type'=>$type,'orderby'=>$order,'order'=>$sort,'post_per_page'=>$num);
                  }
            ...
            <p> <?php _e('Post Type','vibe');?> : <select class="select" name="<?php echo $this->get_field_name('type'); ?>">
                ...
                <p>
                    <?php _e('Style','vibe'); ?> : <select class="select" name="<?php echo $this->get_field_name('style'); ?>">
                        ...
                        <option value="testimonial" <?php if($instance['style'] == 'testimonial') { ?>selected="selected" <?php } ?>><?php _e('Tesimonial (Only for Testimonials)','vibe'); ?></option>
